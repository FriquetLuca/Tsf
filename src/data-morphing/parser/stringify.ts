import type { MergeProperties, ParsedRule, ParsedResultType, ParserAllRules, ParsedEnclosedRule, ParsedOverrideRule, Parsed } from "./parse"
import { duplicateString } from "../../utils"
...
export function stringify<T extends ParserAllRules, U extends MergeProperties<ParsedResultType<T>> = MergeProperties<ParsedResultType<T>>>(nodes: (U[])|MergeProperties<Parsed<T>>, options?: StringifyOptions<T, U>, depth: number = 0): string {
    ...
    const space = options?.spacing ? duplicateString("\t", (options?.spaceAmount ?? 1) * depth) : '';
